...
CREATE TABLE IF NOT EXISTS public.charater_database (
  id TEXT PRIMARY KEY,
  grade_level TEXT NOT NULL CHECK (grade_level IN ('3-4', '5-6', '7-8')),
  week INTEGER NOT NULL CHECK (week >= 1 AND week <= 4),
  day INTEGER NOT NULL CHECK (day >= 1 AND day <= 5),
...
ALTER TABLE public.charater_database ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "Character content readable by everyone" ON public.charater_database;
CREATE POLICY "Character content readable by everyone"
  ON public.charater_database FOR SELECT USING (true);
